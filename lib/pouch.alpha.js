  ...
  while (queue.length > 0) {
    ...
    for (var i = 0; i < tree2[1].length; i++) {
      ...
      if (!merged) {
        ...
        var newtree = [], inserted = false;
        for (var n = 0; n < tree1[1].length; n++) {
          if (tree1[1][n][0] < tree2[1][i][0] || inserted) {
            newtree.push(tree1[1][n]);
          } else {
            newtree.push(tree2[1][i]);
            newtree.push(tree1[1][n]);
            inserted = true;
          }
        } 
    ...
    if ((options = arguments[ i ]) != null) {
      ...
      for ( name in options ) {
        ...
        if (!options.hasOwnProperty(name)) {
          //console.log('dropping prop ' + name, copy)
          continue;
        }

        if (/_kvo/.test(name)) {
          console.log("yikes ", name);
          continue;
        }
...
var newextend = function (deep) {
  return oldExtend.call(arguments);

  var args = deepCopy(Array.prototype.slice.call(arguments, 1), true), src,
  ...
  console.log('extend', args, target, oldExtend.apply(this, arguments));
